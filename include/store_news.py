...
def log_pipeline_metadata(data_dir: str, db_name: str, dag_run_id: str, execution_timestamp: str,
    ...
    dag_logger.info(f"Logging pipeline metadata for DAG Run ID: {dag_run_id}")
    db_path = get_db_path(data_dir, db_name)
    conn = None
    try:
        conn = sqlite3.connect(db_path)
        ...
        conn.commit()
        dag_logger.info("Pipeline metadata logged successfully.")
    except sqlite3.Error as e:
        dag_logger.error(f"Failed to log pipeline metadata: {e}")
    finally:
        if conn:
            conn.close()